...
/*
 设置TableView的大小
 */
CCSize SpCurrency::getTableSize()
{
...
/*
 设置TableView货币一行的坐标
 */
CCSize SpCurrency::cellSizeForTable(CCTableView *table)
{
    return CCSizeMake(G_SF(230), getTableSize().height/3);
    
}

void SpCurrency::createTableViewCell(CCNode *cell,CCTableView *table,int index)
{
//    CLMainSceneLayer* pMainLayer = (CLMainSceneLayer*)CLSceneManager::sharedSceneManager()->getpRunningScene();
    ...
    //货币数量
    int nCardNumber = bl->m_bkData["Shop2"].size();
    ...
    //价格类型
    LeftSalaType = bl->m_bkData["Shop2"][iTag][1].asIntModestyle();
    
    //单价
    LeftMoney = bl->m_bkData["Shop2"][iTag][2].asStringModeStyle();
    ...
    if(itBack != backMap.end()){
        ...
        No = bl->m_bkData["Shop2"][iTag][ZERO].asIntModestyle();
    ...
    //道具IconId
    int ArtIconId = 0;
    ArtIconId = JsonDataManager::shardJDManager()->m_vCardJueseDaoju[bl->m_bkData["Shop2"][iTag][ZERO].asIntModestyle()]->m_artNo;
    ...
    btnLeft->setOnClickCallbackNode(callfuncN_selector(SpCurrency::openView), this);
    ...
    if((index * 2 + 1) < nCardNumber){
        ...
        //价格类型
        RightSalaType = bl->m_bkData["Shop2"][iTag][1].asIntModestyle();
        
        //单价
        RightMoney = bl->m_bkData["Shop2"][iTag][2].asStringModeStyle();
        ...
        btnRight->setOnClickCallbackNode(callfuncN_selector(SpCurrency::openView), this);
...
//table cell选中
void SpCurrency::tableCellTouched(CCTableView* table, CCTableViewCell* cell)
{
//    CLMainSceneLayer* pMainLayer = (CLMainSceneLayer*)CLSceneManager::sharedSceneManager()->getpRunningScene();
    CLSceneLayer* pMainLayer = (CLSceneLayer*)CLSceneManager::sharedSceneManager()->getpRunningScene();
    if(pMainLayer->getnCTIndex() != CLScene_MainScene && pMainLayer->getnCTIndex() != CLScene_ShelfLayer)
    ...
    //价格类型
    salaType = bl->m_bkData["Shop2"][n_Tag][1].asIntModestyle();
    
    cardID = bl->m_bkData["Shop2"][n_Tag][ZERO].asIntModestyle();
    
    price = bl->m_bkData["Shop2"][n_Tag][2].asIntModestyle();
    
    //拥有数量
    haveNumber = bl->m_bkData["Shop2"][n_Tag][3].asIntModestyle();
        ...
        if(salaType != 3){
            ...
            if(userCurrentMoney < price){
                ...
                if(buyMaxNumber == 0)
                {
                    ...
                    ShopBuyLayer *lBuy = ShopBuyLayer::createLayer(cardID,salaType ,price, false,buyMaxNumber,"Shop2");
...
CCTableViewCell* SpCurrency::tableCellAtIndex(CCTableView *table, unsigned int idx)
{
...
//卡牌数量
unsigned int SpCurrency::numberOfCellsInTableView(CCTableView *table)
{
    CLMainSceneLayer* pMainLayer = (CLMainSceneLayer*)CLSceneManager::sharedSceneManager()->getpRunningScene();
    ...
    //货币数量
    int num = bl->m_bkData["Shop2"].size();
...
//查看货币信息
void SpCurrency::openView(CCNode* pSender)
{
...
//    CLMainSceneLayer* pMainLayer = (CLMainSceneLayer*)CLSceneManager::sharedSceneManager()->getpRunningScene();
    ...
    No = bl->m_bkData["Shop2"][pSender->getParent()->getTag()][ZERO].asIntModestyle();
...
//返回正面
void SpCurrency::ClickView(CCNode* pSender,CCNode* pSender2,int tag)
{
...
void SpCurrency::onEnter()
{
    CCLayer::onEnter();
    CCDirector::sharedDirector()->getTouchDispatcher()->addTargetedDelegate(this, -10000, true);
}

void SpCurrency::onExit()
{
    CCLayer::onExit();
    CCDirector::sharedDirector()->getTouchDispatcher()->removeDelegate(this);
}

bool SpCurrency::ccTouchBegan(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent)
{
    // 记录触摸起始点的位置
...
void SpCurrency::ccTouchMoved(CCTouch *pTouch, CCEvent *pEvent)
{
    
}

void SpCurrency::ccTouchEnded(CCTouch *pTouch, CCEvent *pEvent)
{
