...
{
    public:
        enum ErrorCodes { errNone, errBadHostName, errNoIPAddress, errDNSError,
...
class SocketBase
{
    ...
    public:
		...
		virtual		   void OnDisconnect(SocketException exception) {};

        int            Connect(IPAddress &cServAddr, int iServPortNo);
        int            Bind(IPAddress &cLocalAddr, int iLocalPort = 0);
        int            Bind(int iLocalPort = 0);
        int            Listen(int iBackLog);
        int            Accept(SocketBase *pcConnection, IPAddress &cRemoteAddr, int &iRemotePortNo);
        ...
        int            GetSockName(IPAddress &cLocalAddr, int &iLocalPort);
        int            GetPeerName(IPAddress &cRemoteAddr, int &iRemotePort);

        int            GetSockOpt(int iCodeLevel, int iOptionName, void *pOptionData, int &iDataLength);
        int            SetSockOpt(int iCodeLevel, int iOptionName, const void *pOptionData, int iDataLength);
...
class TCPClientSocket : public TCPSocket
{
    public:
		...
		virtual void OnDisconnect(SocketException _exception);
		SocketException & getException() { return exception;}
protected:
