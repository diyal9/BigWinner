...
#define IPNAME_TO_HOST(pheDetails, pcHostName, iAddressType, iError)          \
	{                                                                         \
	pheDetails = gethostbyname(pcHostName);                \
	iError = h_errno;                                                     \
	}
...
inet_ntop6(const struct in6_addr *addr, char *dst, size_t size)
{
	const unsigned short *puiAddress = (unsigned short *)addr->s6_words;
	int             iColon = 0;
	char            *pcBuffer = dst;
	bool            bProcessNumber;
...
inet_ntop4(const struct in_addr *addr, char *dst, size_t size)
{
	const char  *pucAddress = (char *) &(addr->s_addr);
	char  *pcBuffer = dst;
...
const char *
_inet_ntop(int af, const void *addr, char *dst, size_t size)
{
	switch (af)
	{
	case AF_INET6:  return inet_ntop6((in6_addr *)addr, dst, size);
...
IPAddress::privSetAddress(const char *pcNewAddress, const int iNewAddressLength)
{
    ...
    //memcpy(pcAddress, pcNewAddress, iAddressLength*sizeof(int));
...
#else
		_inet_ntop(iAddressType, pcAddress, pcStrAddress, INET6_ADDRSTRLEN);
#endif
...
IPAddress::operator==(const IPAddress &cOtherAddr) const
{
    return ((iAddressType == cOtherAddr.iAddressType) &&
            ...
            (strncmp(pcAddress, cOtherAddr.pcAddress, iAddressLength) == 0));
}
...
IPAddress::SetHostName(const char *pcNewHostName, bool bIPv6)
{
    ...
    if (pheDetails == NULL)
    {
        ...
        switch (iError)
        {
            ...
            default:             return;//throw SocketException(SocketException::errUnknown, iError);
        }
    }
...
IPAddress::ConvertToAddressFamily(int iNewAddressFamily)
{
    ...
    switch (iNewAddressFamily)
    {
        case AF_INET:   if (!(IN6_IS_ADDR_V4MAPPED((struct in6_addr *)pcAddress)))
                        {
                            return;//throw SocketException(SocketException::errCannotConvertToIPv4);
                        }
        ...
        case AF_INET6:  iAddressLength = sizeof(in6_addr);
                        ...
                        break;
        default:        return;//throw SocketException(SocketException::errNoProtocolSupport);
    }
