...
TCPSocket::SendData(const void *pData, int iDataLen, unsigned int uiFlags)
{
    if (!bIsConnected)
    {
		OnDisconnect(SocketException::errNotConnected);
    }
...
TCPSocket::SendBinary16Bits(const int iData, unsigned int uiFlags)
{
    ...
    {
        OnDisconnect(SocketException(SocketException::errUnknown));
    }
}
...
TCPSocket::LocalIPAddress()
{
    if (!bLocalSet)
    {
        OnDisconnect(SocketException(SocketException::errNotBound));
    }
...
/*                                                                            */
/* This method is a wrapper method for the protected base class IoCntl() method*/
/******************************************************************************/
int
TCPSocket::IoCntl(int iCommand, long* lArgument)
{
    return SocketBase::IoCntl(iCommand, lArgument);
}
...
{
	exception = SocketException(SocketException::errNone);
...
void TCPClientSocket::OnDisconnect(SocketException _exception)
{
	exception = _exception;

	if (SocketException::ErrorCodes(_exception) == SocketException::errNotConnected)
		bIsConnected = false;
	//cocos2d::CCLog("OnDisconnect: %s %d", (const char*)_exception, SocketException::ErrorCodes(_exception));
}
